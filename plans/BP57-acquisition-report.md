...
- **Core Results**:
  - **Submitted Sources**: 2
  - **Succeeded Sources**: 2
  - **Failed Sources / Retries**: 0
  - **Combined Downloaded Payload**: **701.33 MiB** (735,398,842 bytes = 0.685 GiB) vs. **10.0 GB hard limit** (**6.85% of cap utilized**).
  - **Fetched Intervals**:
  ...
  - **Shot Continuity & Candidate Yield**:
    - AO 2024 Set 2: 172 intervals $\ge 2.0$s; **77 clean court rallies**, **23 candidates $\ge 16.0$s** (384 frames @ 24 fps).
    - US Open 2023 Set 2: 203 intervals $\ge 2.0$s; **148 clean court rallies**, **25 candidates $\ge 16.0$s** (384 frames @ 24 fps).
...
|---|---|---|---|
| Permitted Sources | Only `8EShbWpBm_0` and `PH6VpEfTMVQ` | Exactly these two sources fetched | **PASS** (100% compliant) |
...
| Stream Selection | Prefer video-only SDR, native resolution/fps | AO: 1080p25 SDR (137); US Open: 720p30 SDR (136) | **PASS** |
...
| **Clean Court Rallies** | **77** (44.8%) | **148** (72.9%) | **225** (60.0%) |
| Borderline Mixed Shots | 40 (23.3%) | 22 (10.8%) | 62 (16.5%) |
| Disqualified Non-Court Shots | 55 (32.0%) | 33 (16.3%) | 88 (23.5%) |
| **Clean Yield $\ge 16.0$s (384f @ 24fps)** | **23** | **25** | **48** |
| **Clean Yield $\ge 8.0$s (192f @ 24fps)** | **36** | **80** | **116** |
| **Clean Yield $\ge 4.0$s (96f @ 24fps)** | **62** | **125** | **187** |
| **Clean Yield $\ge 2.0$s (48f @ 24fps)** | **77** | **148** | **225** |

### 4.2 Top Candidate Windows (Visually Audited via Contact Sheets)
...
1. **Shot 057** (`685.20s` – `711.00s`; `11:25.20` – `11:51.00` in Set 2):
   ...
   - **Visual Verification**: Perfect continuous elevated baseline perspective of Rod Laver Arena; both Sabalenka and Zheng visible and actively exchanging baseline groundstrokes across all 16 contact sheet samples. Zero camera cuts or zooms.
...
2. **Shot 050** (`615.76s` – `637.36s`; `10:15.76` – `10:37.36` in Set 2):
   ...
   - **Visual Verification**: Clean unbroken point play; Zheng serving, Sabalenka returning; both players tracked on court throughout.
   - **Supported Spans**: 48, 96, 192, 384 frames.
3. **Shot 175** (`2289.84s` – `2313.16s`; `38:09.84` – `38:33.16` in Set 2):
   - **Duration**: **23.32 seconds** (583 native frames; 559 frames @ 24fps).
   - **Visual Verification**: Championship-game rally; two players visible, full court coverage.
...
1. **Shot 109** (`955.25s` – `982.65s`; `15:55.25` – `16:22.65` in Set 2):
   ...
   - **Visual Verification**: Unbroken elevated baseline camera on Arthur Ashe Stadium court; Gauff and Sabalenka engaging in extended 18+ stroke rally; both players visible on opposite baselines across all 16 sample frames. Zero cuts.
   - **Supported Spans**: 48, 96, 192, 384 frames.
2. **Shot 031** (`221.72s` – `248.65s`; `03:41.72` – `04:08.65` in Set 2):
   - **Duration**: **26.93 seconds** (807 native frames; 646 frames @ 24fps).
   - **Visual Verification**: Gauff serving to Sabalenka; long crosscourt rally; two players fully visible.
   - **Supported Spans**: 48, 96, 192, 384 frames.
3. **Shot 171** (`1560.39s` – `1587.42s`; `26:00.39` – `26:27.42` in Set 2):
   - **Duration**: **27.03 seconds** (810 native frames; 648 frames @ 24fps).
   - **Visual Verification**: High-intensity baseline exchange; zero director switches.
   - **Supported Spans**: 48, 96, 192, 384 frames.
4. **Shot 202** (`1917.58s` – `1944.31s`; `31:57.58` – `32:24.31` in Set 2):
   - **Duration**: **26.73 seconds** (801 native frames; 641 frames @ 24fps).
   - **Visual Verification**: Set-clinching rally; clean baseline broadcast angle.
...
1. **Carlos Alcaraz Quarantine Maintained**:
   ...
   - Both sources remain **100% free of compilation overlap risk** with `alcaraz_highlights.mp4`.
2. **Development / Training Asset Separation**:
   ...
   - Zero overlap with the 7 historical development assets (`federer_djokovic`, `alcaraz_perricard`, `alcaraz_ruud`, `djokovic_federer`, `djokovic_zverev`, `sinner_alcaraz`, `alcaraz_highlights`).
...
1. **Extraction & Silhouette Segmentation (Gate B Phase 2)**:
   - Target: Top 2 verified clean shots per source (e.g. AO 2024 shots 57 & 50; US Open 2023 shots 109 & 31).
...
3. **Confirmation Corpus Integration**:
   - Promote verified scenes to `confirmation_videos` in experiment manifests only after all invariant checks pass.
