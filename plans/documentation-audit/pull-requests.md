# Pull request index — snapshot through #72

Fetched from the authenticated GitHub PR API on 2026-09-08. All 72 PRs, including closed-unmerged #5; titles below are API titles, not inferred commit subjects. Area tags are audit classifications and may be refined. Merged does not mean a result is citable. This table locates history; read the PR discussion and pinned source before reusing a finding.

The migration plan is [DOCUMENTATION-OVERHAUL.md](../DOCUMENTATION-OVERHAUL.md). In the final layout move this index to `docs/history/pull-requests.md`; link it from AGENTS and each area. Update when a PR is opened/closed/merged, and refresh stale titles/states during an area reconciliation. Keep direct commits in a separate area-evidence row, never invent a PR.
...
| [#5](https://github.com/emanuele-artioli/PointStream/pull/5) | refactor(pipeline): Implement vector-based input for generative models | CLOSED | — |
