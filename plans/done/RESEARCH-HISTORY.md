...
Historical research record, preserved on 2026-09-03. Numbered sections below are
legacy references, not the current task list. Read `../../PLAN.md` and
`../ROADMAP.md` for current status and scheduling.
...
**Integration status:** `plans/done/BP47-integration.md` tracks the consolidated
BP44–BP46 merge (PR #52) and remaining checks. `plans/done/BP48-recovery-validation.md`
...
and a text prompt. `plans/done/RESEARCH-HISTORY.md` §6.6's cost order — tune, then fine-tune, then
...
`plans/done/BP9-probe-harness.md` made the static-copy floor a permanent arm
(`phase-bp/bp9`).
...
`plans/done/BP17-caption-channel.md`.
...
`plans/done/BP20-headroom-real-ladder.md` is the replacement.
...
array size. `plans/done/RESEARCH-HISTORY.md` §2.16 recorded the consequence: no compression ratio could
...
**Read `plans/done/BP24-findings.md` before quoting any rate.** Eleven findings,
...
`BP24` continued, 2026-08-28. Report: `plans/done/BP24-ladder-report.md`.
...
`BP24` concluded, 2026-08-28. `plans/done/RESEARCH-HISTORY.md` §7 **P0 items 2 and 3 are closed.**
Report: `plans/done/BP24-ladder-report.md`. Bounds written before the first encode:
`outputs/bp24-ladder/bounds-before-run.json`.

**Paired arms, one codec on both, same preset** — the design
`plans/done/BP24-findings.md` §1 settled on, so the preset cancels. Y-PSNR, which is
...
are in `plans/done/BP24-findings.md` §§8, 14 and 2.
...
2026-08-29, following §2.20. Briefs: `plans/done/BP29-plate-rate.md` (where
PointStream can win) and `plans/done/BP30-background-stream.md` (the background as a
stream). Findings: `plans/done/BP24-findings.md` §§16-18.
...
`plans/done/BP31-findings.md` §10; `outputs/bp31-ladder/plate-codec-curves-alcaraz_highlights.json`.
...
`plans/done/BP30-findings.md` §§20-28. Bounds written before the first encode:
...
**Next: `plans/done/BP31-paired-ladder-across-scenes.md`** — the paired ladder over N
...
change in meaning is visible rather than silent (`plans/done/BP24-encoder-boundary.md`
step 3).
...
2. ✅ **DONE 2026-08-28** — PointStream against the codec ladder, as paired
   ...
   not overlap. §2.20, `plans/done/BP24-ladder-report.md`. *Region arms are not in
...
8. **The plate.** It is 88-91% of PointStream's payload at every rung (§2.20),
   ...
   **not** grow, so the whole gain is the residual. On a near-static clip the
   ...
   `plans/done/BP29-panorama-report.md`.
