...
from src.components.selection.identity import IdentitySelector
...
def test_identity_keeps_every_detection() -> None:
    """General-domain default: no tennis two-player cap, no crowd filter."""
    assert SELECTION.spec("identity").name == "identity"
    selector = SELECTION.build("identity")
    assert isinstance(selector, IdentitySelector)
    detections = [
    ...
    selected = selector.select(detections, (720, 1280))
    assert [item.track_id for item in selected] == [
        "far_track",
        "near_track",
        "kid_track",
    ]
