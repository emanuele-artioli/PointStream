...
def _collect_tracked_objects(scene: Scene) -> Dict[int, TrackedObjectData]:
    """Gathers all frames and bounding boxes for each unique track ID."""
    tracked_objects: Dict[int, TrackedObjectData] = {}
    
    # To process objects, we need the original frames again.
    # This requires re-reading the relevant part of the video.
    ...
    for _ in range(scene['end_frame'] - scene['start_frame'] + 1):
        ret, frame = cap.read()
        if not ret: break
        frames.append(frame)
    cap.release()
    ...
    for frame_idx, frame_detections in enumerate(scene['detections']):
        for detection in frame_detections:
            ...
            bbox_abs = [
                int(bbox_norm[0] * width), int(bbox_norm[1] * height),
...
def _extract_feature_keypoints(frames: List[np.ndarray], bboxes: List[List[int]]) -> List[np.ndarray]:
    """Placeholder for rigid object keypoint extraction."""
...
def run_foreground_pipeline(scene_generator: Generator[Scene, None, None], video_path: str) -> Generator[Scene, None, None]:
    ...
    for scene in scene_generator:
        ...
        scene['video_path'] = video_path # Add video_path for frame re-reading
        ...
        if scene['motion_type'] == 'COMPLEX' or not scene.get('detections'):
            ...
            for track_id, data in tracked_objects.items():
                ...
                frames, bboxes = data['frames'], data['bboxes_abs']
                keypoints, keypoint_type = [], "none"

                if class_name == 'person':
                    print(f"     -> Extracting MMPose keypoints for track ID {track_id}...")
                    keypoints = mmpose_handler.extract_poses(frames, bboxes, 'person')
        ...
        if 'video_path' in scene: del scene['video_path']
