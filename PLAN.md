...
not using appearance. Epochs 1–5 all landed in **11.18–11.33 dB** (static-copy
letterbox floor 11.47 on this driver). The series is flat-to-down (epoch 1
11.33 → epoch 5 11.18). That band is evidence the reference paint is not
teaching identity; epoch 10 is the number that decides the roster, not a
...
|---|---|---|---|
| **Quality flagship** | **Unset.** Pose-ref ControlNet is still training (epoch 10 decides). Epochs 1–5 did not beat static copy. | `eval-ladder` | AA 8.96 dB in-domain, real IP-Adapter 8.90 dB, published pose/seg ControlNet 11.20 / 11.01, pose-ref epochs 1–5 **11.33 / 11.25 / 11.24 / 11.24 / 11.18** — all at or below the 11.82 dB static-copy floor (this driver's letterbox floor 11.47). An engine at or below that floor is not using appearance. |
...
| **ControlNet family** | **Keep as comparison backbone; quality flagship pending pose-ref epoch 10** | pose 11.20 / seg 11.01 (published, lose to static). `ip-adapter-controlnet` directory is a mislabelled seg ControlNet — do not load it. Real IP-Adapter 8.90 dB, **not using appearance**. Pose-ref epochs 1–5: 11.33 / 11.25 / 11.24 / 11.24 / 11.18 dB, all **not using appearance**. |
