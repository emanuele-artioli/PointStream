...
| **Missing Baselines**<br>Omission of AV1/VVC and recent learned codecs (DVC, NeRV, SVC-LC). | R2, R5 | 🟡 **Partially Tackled**<br>AV1/HEVC full-length anchor curve and a High-Capacity HNeRV baseline (with VOD Weight Residual Coding) both done with real numbers ([9](9_codec_baselines_report.md)); VVC anchor and the actual PointStream-vs-anchors overlay still open. | Run PointStream itself against the AV1/HEVC/HNeRV curves via `--pointstream-run`; add a VVC anchor if `vvenc`/`libx266` becomes available. Add tables to the Results section. | Medium |
...
- [x] **Learned Codec Baselines:** Benchmark against at least one recent semantic codec (e.g., DVC or NeRV). **Done (2026-07-12):** High-Capacity HNeRV trained with Weight Residual Coding (Model Delta Streaming) for VOD on `real_tennis.mp4`. The 60-frame payload was 39.2 MB vs 2.5 MB for AV1, conclusively proving HNeRV is uncompetitive on bytes. The comparison now exists with real numbers ([9](9_codec_baselines_report.md)).
