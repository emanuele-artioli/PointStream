...
- **Codex**: Start ordinary PointStream sessions with Terra/medium. Use `.codex/config.toml` profiles only when they replace work: `luna_medium` for an independent bounded aid and `sol_medium` for targeted review/repair after a declared failed check. Use Astra only for milestone planning that produces a compact durable brief. The [demo trial](subagent-ladder-trial.md) compares this with Astra-led delegation.
