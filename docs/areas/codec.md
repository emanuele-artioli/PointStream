...
1. **Scorer calibration**: Validated against identity, mild/severe blur and noise, and unrelated image across visible, object, boundary, and full-frame windowed scopes (`scorer_calibration.json`). All anchor orderings hold (0 alarms). Empty-mask behavior returns `float('nan')` cleanly without division-by-zero or slice mean warnings.
2. **Standalone client decoding**: Decoded all 6 saved E04A bitstreams strictly from bitstream bytes and packed side data (`.bin`) with zero geometry inputs available (`standalone_decode_report.json`). Bit-identical parity (max diff = 0) verified across all 6 settings. Rejection of truncated and extra video frames without silent padding/clipping verified and covered by regressions in `tests/test_background_probe_standalone.py`.
...
4. **Timing strata & host provenance**: Separated common preparation (1.81 s) from per-arm encoding. Measured foreground compositing stratum (0.0437 s for 48 frames, 0.91 ms/frame) and reported both client background decode time and total client display time. Recorded explicit host provenance (`gpu5` for E04A, `gpu6` for E03B).
...
6. **Next-probe proposal**: Formulated smallest costed second-camera plus paired-removal experiment (`second_camera_proposal.json`: scenes 001 and 007, removal on vs off, 24 total runs, CPU-only ~180s, awaiting review).
