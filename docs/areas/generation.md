...
GPU diagnostic matrix (gpu5, 16 frames, pix2pix, residual QP 32): gen-off and gen-on **without residual produced identical PSNR/SSIM/VMAF**; generation stage logged 0.7–1.2 s. Residual-on moved quality (~+8 dB PSNR-Y on the paste arm). Not a model ranking. Staged training remains unlaunched until generation is shown to change the delivered frames.
