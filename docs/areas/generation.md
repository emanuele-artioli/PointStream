...
- **E05 Stage 1 Executable Card**: Published `docs/workflow/session/evaluation-campaign/tasks/05-decision-card.md` establishing the executable Stage 1 pilot protocol with corrected first-reference policy, tiny-scene learnability check, disjoint development validation, paste/warp controls, charged total bytes accounting, 3D metric reporting (rate, quality, latency), explicit uncertainty-aware promotion criteria, and an aggregate 1.0 GPU-hour cap. Removed circular pre-run release gates and quarantined confirmation sources; returned ready for pilot release without launching yet.
