...
* **Concurrent Session Commit Discipline:** This repo gets worked on by multiple agent sessions at once — Claude (interactive or spawned tasks, which do use isolated worktrees) and other tools that read their own copies of these rules here (Antigravity, Copilot). Any session working directly in the main checkout (not an isolated worktree) can silently overwrite another's uncommitted edit on the same file via an ordinary read-modify-write race. Commit a fix as soon as it passes fast checks (ruff/mypy/unit tests) — never leave it uncommitted while running a slow verification (a multi-minute integration test, a real GPU/pipeline run) or while moving to unrelated work. A surprising empty `git diff --stat` on a file you just edited is the tell that this happened — re-apply and commit immediately rather than diagnosing why.  
...
* **Research Documentation — the paper is the primary living document:** the manuscript `67a9ea6275d3d9785ce57026/main.tex` carries the research plan as machine-readable comment markers (`STATUS/GOAL/HOLE/NOTE/NEXT/CLAIM(anchor):` — spec in that repo's `CLAUDE.md`), and `67a9ea6275d3d9785ce57026/RESEARCH_LOG.md` is the secondary store (hard rules, standing results, bug/dead-end/superseded registries, asset inventory). **The 13 numbered `reports/*.md` were consolidated into these two files on 2026-07-21 and deleted** — do not recreate a `reports/` tree; history is in `git log --follow -- reports/<file>`. Before planning an experiment, grep the paper's `HOLE()` markers and run only what the paper needs. After committed, tested results land, clear the `HOLE` and write the `CLAIM(id): src=<outputs path>` line in the same edit.
* **Hard experiment rules (each has already produced a wrong conclusion):** (1) the encoder computes residuals against the *codec-decoded* panorama, never the raw one — any new synthesis path gets a bit-identity check; (2) verify a config knob is actually wired before ablating it (grep its consumer, not the schema) — an unwired threshold once produced an entirely fictional ablation table; (3) infra failure is not a quality result — never rank or prune a training rung where an alive variant crashed or OOM'd; (4) no generative quality claim unless the model was trained without the held-out `alcaraz_highlights` and `djokovic_zverev`; (5) "conclusively"/"definitively" is banned on single-clip, single-architecture results; (6) codec presets are not comparable by name — compare at matched VMAF across a CRF ladder; (7) every number cites an `outputs/<timestamp>/` path and states its config (the default caps at `num-frames: 10`); (8) invalidated runs are `mv`'d to `outputs/_superseded/`, never deleted — and untracked scratch files are not in git either, so read anything before removing it.
